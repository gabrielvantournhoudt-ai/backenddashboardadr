...
def fetch_yahoo_data(ticker, is_adr=False):
    ...
    try:
        ...
        safe_ticker = urlquote(ticker, safe='=.-_')
        url = f"https://query1.finance.yahoo.com/v8/finance/chart/{safe_ticker}?range=3d&interval=15m&includePrePost=true"
        headers = {
        ...
        if 'chart' in data and 'result' in data['chart'] and data['chart']['result']:
            ...
            if is_adr:
                ...
                # Remover duplicados e zero/NaN
                cleaned_prev = []
                for cand in prev_candidates:
                    try:
                        fv = float(cand)
                        if fv and fv > 0 and (not cleaned_prev or abs(cleaned_prev[-1] - fv) > 1e-9):
                            cleaned_prev.append(fv)
                    except Exception:
                        continue
                prev_value = cleaned_prev[0] if cleaned_prev else None
                ...
                # Se ainda não há variação, tentar usar os dois últimos valores válidos da série
                if (variation is None or abs(variation) < 1e-9) and series_closes and len([c for c in series_closes if c is not None]) >= 2:
                    try:
                        last_vals = [float(c) for c in series_closes if c is not None][-2:]
                        if last_vals[0] != 0:
                            variation = ((last_vals[1] - last_vals[0]) / last_vals[0]) * 100
                        if current_price is None:
                            current_price = last_vals[1]
                    except Exception:
                        pass

                # Fallback final: usar dados diários (até 5d) para calcular variação entre closes diários
                if variation is None or abs(variation) < 1e-9:
                    try:
                        daily_url = f"https://query1.finance.yahoo.com/v8/finance/chart/{safe_ticker}?range=5d&interval=1d&includePrePost=true"
                        headers = {
                            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
                        }
                        req_daily = urllib.request.Request(daily_url, headers=headers)
                        with urllib.request.urlopen(req_daily, timeout=10) as resp_daily:
                            daily_data = json.loads(resp_daily.read().decode())
                        if 'chart' in daily_data and 'result' in daily_data['chart'] and daily_data['chart']['result']:
                            d_result = daily_data['chart']['result'][0]
                            d_ind = (d_result.get('indicators', {}) or {}).get('quote', [{}])
                            d_closes = (d_ind[0] if d_ind else {}).get('close', []) or []
                            d_vals = [float(c) for c in d_closes if c is not None]
                            # pegar os dois últimos valores distintos
                            distinct = []
                            for v in reversed(d_vals):
                                if not distinct or abs(distinct[-1] - v) > 1e-9:
                                    distinct.append(v)
                                if len(distinct) == 2:
                                    break
                            if len(distinct) == 2 and distinct[1] != 0:
                                # distinct[0] é o último, distinct[1] o anterior distinto
                                variation = ((distinct[0] - distinct[1]) / distinct[1]) * 100
                                if current_price is None:
                                    current_price = distinct[0]
                            # Se ainda insuficiente, ampliar janela para 1 mês
                            if (variation is None or abs(variation) < 1e-9) and len(d_vals) < 2:
                                month_url = f"https://query1.finance.yahoo.com/v8/finance/chart/{safe_ticker}?range=1mo&interval=1d&includePrePost=true"
                                req_month = urllib.request.Request(month_url, headers=headers)
                                with urllib.request.urlopen(req_month, timeout=10) as resp_m:
                                    month_data = json.loads(resp_m.read().decode())
                                if 'chart' in month_data and 'result' in month_data['chart'] and month_data['chart']['result']:
                                    m_res = month_data['chart']['result'][0]
                                    m_ind = (m_res.get('indicators', {}) or {}).get('quote', [{}])
                                    m_closes = (m_ind[0] if m_ind else {}).get('close', []) or []
                                    m_vals = [float(c) for c in m_closes if c is not None]
                                    distinct_m = []
                                    for v in reversed(m_vals):
                                        if not distinct_m or abs(distinct_m[-1] - v) > 1e-9:
                                            distinct_m.append(v)
                                        if len(distinct_m) == 2:
                                            break
                                    if len(distinct_m) == 2 and distinct_m[1] != 0:
                                        variation = ((distinct_m[0] - distinct_m[1]) / distinct_m[1]) * 100
                                        if current_price is None:
                                            current_price = distinct_m[0]
                    except Exception:
                        pass
